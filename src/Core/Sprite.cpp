...
Sprite::Sprite() : GameObject(), Mesh(), m_Texture(nullptr), m_Width(0), m_Height(0)
{
	//
}

Sprite::Sprite(std::string &aFilePath) : GameObject(), Mesh(), m_Texture(nullptr), m_Width(0), m_Height(0)
{
	LoadTexture(aFilePath);
}

Sprite::Sprite(std::string &aFilePath, glm::vec3 aPosition, glm::vec3 aRotation, glm::vec3 aScale, bool aIsEnabled) : GameObject(aPosition, aRotation, aScale, aIsEnabled),
...
bool Sprite::LoadTexture(std::string &aFilePath)
{
	if (m_Texture == nullptr)
	{
...
void Sprite::Draw()
{
	...
	if (uWVP)
	{
		...
		//matrix = glm::perspective(45.0f, 640.0f / 480.0f, 0.01f, 100.0f) * matrix * view;
...
void Sprite::SetAnchorPoint(glm::vec2 aAnchorPoint)
{
	m_AnchorPoint - aAnchorPoint;
}
...
void Sprite::SetWidth(float aWidth)
{
	m_Width = aWidth;
}
...
void Sprite::SetHeight(float aHeight)
{
	m_Height = aHeight;
}
